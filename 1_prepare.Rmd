...
- Avec le recodage des auteurs, les NAs sont conséquentes : 1928 en tout, affectant 828 individus (avec une moyenne de 1,6 NA par individu) et représentant jusqu'à 20-30 % des données pour certaines variables.
...
Données finales
- B&D éliminent les individus avec plus de 10 NAs pour les outcomes, et au moins 1 NA pour les contrôles.
- Nous : on élimine l'individu avec seulement 2 réponse pour les outcomes, et on impute les valeurs manquantes par kNN

```{r}
if (approach == 'replic') {
  data2004_final_ni <- data2004_recoded %>%
    filter(rowSums(is.na(select(., all_of(f_outcomes)))) < 10) %>%
    filter(rowSums(is.na(select(., all_of(all_controls)))) == 0)
  data2004_final <- data2004_final_ni %>% 
    kNN(k=5, imp_var = FALSE)
}
```

```{r}
if (approach == 'own') {
  data2004_final <- data2004_recoded %>%
  filter(id != 769) %>% 
  kNN(k=5, imp_var = FALSE)
}
```
...
write_parquet(data2004_final, paste0("data/data2004_", approach, ".parquet"))
write.csv(data2004_final, paste0("data/data2004_", approach, ".csv"))

if (approach == 'replic') {
  write_parquet(data2004_final_ni, "data/data2004_replic_ni.parquet")
  write.csv(data2004_final_ni, "data/data2004_replic_ni.csv")}
```
